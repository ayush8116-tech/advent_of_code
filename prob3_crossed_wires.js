...
const add = (currentPos, offset, distance) => {
  const tracedPath = [];
  ...
  for (let i = 0; i < distance; i++) {
    ...
    tracedPath.push(newPoint);
  }

  return tracedPath;
};

const pointAdder = (current, [direction, distance]) => {
  const addingOffset = offset[direction];
  const nextPosition = add(current, addingOffset, distance);

  return nextPosition;
};

const tracer = (instructions) => {
  let currentPosition = [0, 0];
  const instructionTokens = instructions.split(",");
  const tracedPath = instructionTokens.flatMap((instruction) => {
    const parsedInstruction = [
      [...instruction][0],
      parseInt([...instruction].slice(1).join("")),
    ];

    return pointAdder(currentPosition, parsedInstruction);
  });

  return tracedPath;
};

// console.log(tracer("R10,U15"));

const spotIntersection = ([wire1, wire2]) => {
  const wire1Points = wire1.map((x) => x.join(""));
  const wire2Points = wire2.map((x) => x.join(""));
  const intersected = [];

  for (let i = 0; i < wire1Points.length; i++) {
    for (let j = 0; j < wire2Points.length; j++) {
      if (wire1Points[i] === wire2Points[j]) {
        intersected.push(wire1[i]);
      }
    }
  }
...
const findDistance = (intersections) => {
  const centralPort = [0, 0];
  const [x1, y1] = centralPort;

  return intersections.map((intersection) => {
    const [x2, y2] = intersection;
    return Math.abs(x1 - x2) + Math.abs(y1 - y2);
  });
};

const stepsCounter = (points, tracedPath) => {
  const pathAsString = tracedPath.map((wire) =>
    wire.map((point) => point.join(""))
  );
  const pointsAsString = points.map((point) => point.join(""));
  const [wire1, wire2] = pathAsString;

  const stepsTaken = pointsAsString.map((point) => {
    const intersectionIndex = [
      wire1.findIndex((position) => position === point),
      wire2.findIndex((position) => position === point),
    ];
    const wire1steps = wire1.slice(0, intersectionIndex[0] + 1).length;
    const wire2steps = wire2.slice(0, intersectionIndex[1] + 1).length;
...
const closest = (stepsTaken) => {
  const sumOfSteps = stepsTaken.map(steps => sum(steps))

  return Math.min(...sumOfSteps);
}

const executer = (wires) => {
  const tracedPath = wires.map(tracer);
  const intersections = spotIntersection(tracedPath);
  const distancesFromCentralPort = findDistance(intersections);
  const closestDistance = Math.min(...distancesFromCentralPort);
  const closestIntersection =
    intersections[distancesFromCentralPort.indexOf(closestDistance)];
  const stepsTaken = stepsCounter(intersections, tracedPath);

  return closest(stepsTaken);
};
...
console.log(executer([wireOne, wireTwo]));